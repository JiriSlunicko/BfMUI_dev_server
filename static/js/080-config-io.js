/** Functionality for saving and loading named server-side configurations. */
...
window.serverConfig = (function()
{
  ...
  function init() {
    utils.qs("#settings-configs-type-select").addEventListener("change",
      _updateConfigNameSelect);
    utils.qs("#settings-configs-load").addEventListener("click",
      _.throttle(_loadConfig, 1000, {trailing: false}));
    utils.qs("#settings-configs-save").addEventListener("click",
      _.throttle(_saveConfig, 1000, {trailing: false}));
  }
  ...
  function _updateConfigTypeSelect(selected = "plane") {
    ...
    utils.removeChildren(typeSelect);

    for (const cfgType of _configTypes) {
      typeSelect.insertAdjacentHTML("beforeend", `
        <option value="${cfgType}"${cfgType===selected ? " selected" : ""}
          >${cfgType}</option>`);
    }
  ...
  /** Populate the stored config selector with options for the currently selected config type. */
  function _updateConfigNameSelect() {
    const nameSelect = utils.qs("#settings-configs-name-select");
    utils.removeChildren(nameSelect);
   ...
   */
  function _applyConfig(cfgType) {
    settingsManager.load(_configGroups[cfgType] ?? [])
  }


  /** Save current (server-side!) config under an existing name, or prompt for a new one.
   * 
   * @returns {Promise<boolean>} success
   */
  async function _saveConfig() {
    ...
    // new config -> ask for a name first
    if (cfgName === "__NEW__") {
      const newName = await ui.makePopup(
        "prompt",
    ...
    if (_availableConfigs[cfgType]?.includes(cfgName)) {
      const consent = await ui.makePopup(
        "confirm",
        `The configuration '${cfgName}' already exists. Okay to overwrite?`,
        "Confirm overwrite"
      );
      if (!consent) return;
    }
    ...
    // refresh from the server-side source of truth
